    local EventFrame = CreateFrame("Frame", "EncounterJournalMover", UIParent)
    EventFrame:Hide()
    ...
    EventFrame:SetScript('OnEvent', function(self, event, name)
        if name == 'Blizzard_EncounterJournal' then
            ...
            local EJEncounterJournal = EncounterJournal
            self:SetParent(EJEncounterJournal)
            self:SetSize(EJEncounterJournal:GetWidth()-50, 15) -- -50 to not block the close button
            EJEncounterJournal:ClearAllPoints()
            EJEncounterJournal:SetPoint("TOP", self)
            EJEncounterJournal.ClearAllPoints = function() end
            EJEncounterJournal.SetPoint = function() end
            self:Show()
        end
    end)
