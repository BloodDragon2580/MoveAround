    local EventFrame = CreateFrame("Frame", "VoidStorageFrameMover", UIParent)
    EventFrame:Hide()
    ...
    EventFrame:SetScript('OnEvent', function(self, event, name)
        if name == 'Blizzard_VoidStorageUI' then
            ...
            local VSVoidStorageFrame = VoidStorageFrame
            self:SetParent(VSVoidStorageFrame)
            self:SetSize(VSVoidStorageFrame:GetWidth()-50, 15) -- -50 to not block the close button
            VSVoidStorageFrame:ClearAllPoints()
            VSVoidStorageFrame:SetPoint("TOP", self)
            VSVoidStorageFrame.ClearAllPoints = function() end
            VSVoidStorageFrame.SetPoint = function() end
            self:Show()
        end
    end)
