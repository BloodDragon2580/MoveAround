    local EventFrame = CreateFrame("Frame", "ItemInteractionFrameMover", UIParent)
    EventFrame:Hide()
    ...
    EventFrame:SetScript('OnEvent', function(self, event, name)
        if name == 'Blizzard_ItemInteractionUI' then
            ...
            local IIFItemInteractionFrame = ItemInteractionFrame
            self:SetParent(IIFItemInteractionFrame)
            self:SetSize(IIFItemInteractionFrame:GetWidth()-50, 15) -- -50 to not block the close button
            IIFItemInteractionFrame:ClearAllPoints()
            IIFItemInteractionFrame:SetPoint("TOP", self)
            IIFItemInteractionFrame.ClearAllPoints = function() end
            IIFItemInteractionFrame.SetPoint = function() end
            self:Show()
        end
    end)
