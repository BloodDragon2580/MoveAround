    local EventFrame = CreateFrame("Frame", "ArchaeologyFrameMover", UIParent)
    EventFrame:Hide()
    ...
    EventFrame:SetScript('OnEvent', function(self, event, name)
        if name == 'Blizzard_ArchaeologyUI' then
            ...
            local AFArchaeologyFrame = ArchaeologyFrame
            self:SetParent(AFArchaeologyFrame)
            self:SetSize(AFArchaeologyFrame:GetWidth()-50, 15) -- -50 to not block the close button
            AFArchaeologyFrame:ClearAllPoints()
            AFArchaeologyFrame:SetPoint("TOP", self)
            AFArchaeologyFrame.ClearAllPoints = function() end
            AFArchaeologyFrame.SetPoint = function() end
            self:Show()
        end
    end)
