    local EventFrame = CreateFrame("Frame", "AlliedRacesFrameMover", UIParent)
    EventFrame:Hide()
    ...
    EventFrame:SetScript('OnEvent', function(self, event, name)
        if name == 'Blizzard_AlliedRacesUI' then
            ...
            local ARFAlliedRacesFrame = AlliedRacesFrame
            self:SetParent(ARFAlliedRacesFrame)
            self:SetSize(ARFAlliedRacesFrame:GetWidth()-50, 15) -- -50 to not block the close button
            ARFAlliedRacesFrame:ClearAllPoints()
            ARFAlliedRacesFrame:SetPoint("TOP", self)
            ARFAlliedRacesFrame.ClearAllPoints = function() end
            ARFAlliedRacesFrame.SetPoint = function() end
            self:Show()
        end
    end)
